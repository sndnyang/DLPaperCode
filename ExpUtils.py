...
def init_neptune(name, params, args):
    try:
        import neptune
    except ModuleNotFoundError:
        return

    neptune.init(project_qualified_name='sndnyangd/YOEBM',
                 api_token=os.environ.get('NEPTUNE_API_TOKEN', ''),
                 )
    tags = None
    if args.note:
        tags = [args.note]
    neptune.create_experiment(name=name, params=params, tags=tags)


def init_env(args, exp_logger, cmp_params=None):
    if not cmp_params:
        cmp_params = [k for k, _ in args._get_kwargs()]
    cmp_params += ['hostname', 'pid']
    # 1. debug -> num_workers
    init_debug(args)
    ...
    if 'NEPTUNE_API_TOKEN' not in os.environ:
        return
    if os.environ['NEPTUNE_API_TOKEN'] == '':
        return
    # if not args.debug and args.vis:
    #     params = {k: v for k, v in args._get_kwargs() if k in cmp_params}
    #     args.neptune = True
    #     init_neptune(args.exp_name, params, args)
